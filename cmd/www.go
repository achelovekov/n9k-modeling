...
func (md *MetaData) serviceFootprint(w http.ResponseWriter, r *http.Request) {
	...
	for _, DeviceFootprintDBEntry := range md.ProcessedData["ServiceFootprintDB"].(bson.A) {
		if DeviceFootprintDBEntry.(bson.M)["DeviceName"].(string) == r.FormValue("device") {
			DeviceFootprintDBEntry.(bson.M)["ServiceComponents"] = md.ProcessedData["ServiceComponents"]
			tpl.ExecuteTemplate(w, "serviceFootprint.gohtml", DeviceFootprintDBEntry.(bson.M))
		}
	}
}
...
func main() {
	...
	processedDataCollection := client.Database(config.ServiceName).Collection("ServiceData")
